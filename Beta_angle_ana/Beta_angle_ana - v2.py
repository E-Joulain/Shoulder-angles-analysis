...
class Beta_angle_anaWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onCalculateMetrics(self, timepoint: str):
        """
        Compute beta angle directly from the fiducial nodes in the scene,
        draw lines connecting the points, annotate the angle, and save metrics.
        """
        # Get the fiducial node placed by the user
        nodeName = f"Fiducials_{timepoint}"
        ...
        # Read fiducial positions directly from the markup node
        ...
        # Optionally force z=0 for 2D X-ray slices
        for label in points:
            points[label]['z'] = 0
        ...
        sas_distance = self.logic.distance_point_to_line(P7, P5, P6)

        # Draw lines connecting P1-P2 (scapular) and P3-P4 (glenoid)
      #  self.logic.draw_beta_angle_lines(points)
        self.logic.draw_beta_angle_arc(points)

        # Save metrics to CSV
        metrics = {"beta_angle": beta_angle, "critical_shoulder_angle": csa_angle, "subacromial_sapce": sas_distance}
        self.logic.saveMetricsToCSV(metrics, patientID)

        slicer.util.showStatusMessage(f"Beta angle computed: {beta_angle:.1f}°", 5000)
...
class Beta_angle_anaLogic(ScriptedLoadableModuleLogic):
    ...
    def get_fiducial_positions(self, markupNode):
        """
        Returns a dictionary of fiducial positions keyed by label (e.g., P1, P2, ...)
        """
        points = {}
        if not markupNode:
            return points
    ...
    def generate_arc_points(self, vertex, p1, p2, num_points=30):
        """
        Generate points along an arc between vectors p1->vertex and p2->vertex
        """
        v1 = p1 - vertex
        v2 = p2 - vertex
        ...
        axis = np.array([0, 0, 1])  # rotation axis (XY plane)
        ...
        for t in np.linspace(0, 1, num_points):
            ...
            arc_point_2d = vertex[:2] + rot_matrix @ v1_norm[:2] * np.linalg.norm(v1)
            points.append([arc_point_2d[0], arc_point_2d[1], vertex[2]])
        return points

    def draw_beta_angle_arc(self, points):
        vertex = np.array([points['P2']['x'], points['P2']['y'], points['P2']['z']])
        p1 = np.array([points['P1']['x'], points['P1']['y'], points['P1']['z']])
        p2 = np.array([points['P3']['x'], points['P3']['y'], points['P3']['z']])

        arc_points = self.generate_arc_points(vertex, p1, p2, num_points=30)
        ...
        # Create curve node
        curveNode = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLMarkupsCurveNode", "BetaAngleArc")
    ...
    def distance_point_to_line(self, P, A, B):
        """
        Compute distance from point P to line passing through A and B
        P, A, B are numpy arrays [x,y,z]
        """
        line_vec = B - A
        point_vec = P - A
